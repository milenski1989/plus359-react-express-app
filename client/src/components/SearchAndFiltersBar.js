...
function SearchAndFiltersBar({
    ...
    const getArtists = async () => {
        try {
            const res = await fetch(`https://storage-management-app/artists/relatedToEntriesInStorage/${name.split(':')[1]}`)
    ...
    const getCells = async () => {
        try {
            const res = await fetch(`https://storage-management-app/storage/all/allCellsFromCurrentStorage/${name.split(':')[1]}`)
            const data = await res.json()
