...
function SearchAndFiltersBar({
    ...
    const getArtists = async () => {
        try {
            const res = await fetch(`https://plus359-react-express-lk1cf594n-milenski1989s-projects.vercel.app/artists/relatedToEntriesInStorage/${name.split(':')[1]}`)
    ...
    const getCells = async () => {
        try {
            const res = await fetch(`https://plus359-react-express-lk1cf594n-milenski1989s-projects.vercel.app/storage/all/allCellsFromCurrentStorage/${name.split(':')[1]}`)
            const data = await res.json()
