...
function SearchAndFiltersBar({
    ...
    const getArtists = async () => {
        try {
            const res = await fetch(`https://artworks-management-app.vercel.app/artists/relatedToEntriesInStorage/${name.split(':')[1]}`)
    ...
    const getCells = async () => {
        try {
            const res = await fetch(`https://artworks-management-app.vercel.app/storage/all/allCellsFromCurrentStorage/${name.split(':')[1]}`)
            const data = await res.json()
