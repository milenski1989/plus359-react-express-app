...
function SearchAndFiltersBar({
    ...
    const getArtists = async () => {
        try {
            const res = await fetch(`https://plus359-react-express-app.vercel.app/artists/relatedToEntriesInStorage/${name.split(':')[1]}`)
    ...
    const getCells = async () => {
        try {
            const res = await fetch(`https://plus359-react-express-app.vercel.app/storage/all/allCellsFromCurrentStorage/${name.split(':')[1]}`)
            const data = await res.json()
